...
- 最小 harness 四构件：
  - **观测 Observation**：AI 看得到什么——目录、命名、模板、CLAUDE.md 等信息
  - **工具 Tools**：AI 可以调用什么——run / build / test 脚本
  - **裁判 Judge**：怎么知道做对了——validator、checker、benchmark
  - **记忆 Memory**：下次怎么接着做——progress、iterations、logs、git history
- 另一个看 harness 的维度：从"需求出发"按任务阶段拆
  - **执行前 / 信息**：要获取哪些信息？如何获取？信息和结构怎么组织？
  - **执行中 / 工具**：流程是否要固化？固化哪些部分？工具（含 SKILL）怎么设计？
  - **执行后 / 评测**：怎么评测？评测时间怎么控制？指标怎么选？
  - **结束 / 记忆**：memory 怎么设计？经验怎么沉淀？可观测性做到什么地步？
...
#### 12-20 min | Demo 2：裸 Claude Code 做同一个任务【演示】

在空目录里用 Claude Code 做同一个任务，暴露"会做但不稳"的问题

#### 20-25 min | Demo 2.5：Coding Agent 真正有优势的任务【演示】

换一个 Coding Agent 明显占上风的任务（比如读一个陌生 repo 回答问题、在现有代码里做一次小改并跑通测试），快速展示：
- 本质区别不是模型更聪明，而是 agent 能直接和本地环境交互、过程透明、能处理更复杂的任务
- 所以刚才 Demo 2 的"不稳"不是 coding agent 不行——它换个任务就能赢
- 问题出在"通用环境太松"，这正是引出 harness 的由头
- 顺带澄清：现在的网页版对话式 AI 其实也是 agent，只不过环境是其内部的沙盒；Coding Agent 的优势是直接跑在你的真实环境里

#### 25-32 min | 第一次抽象：引出 harness【讲】
...
#### 32-40 min | Demo 3：揭示 harness + 重跑【演示+讲】
...
#### 40-50 min | Demo 4：迭代 harness【演示】

换一个任务触发失败，不口头纠正，而是现场修 CLAUDE.md / pipeline.sh，展示 harness 从失败中长出来的过程

#### 50-55 min | 第二次抽象：四构件 + 需求视角【讲】

从前面所有 demo 中提炼最小 harness 四构件：**观测 / 工具 / 裁判 / 记忆**（对应"AI 看得到什么 / 能做什么 / 怎么知道做对了 / 下次怎么接"）。
再换一个维度——"从需求出发"按任务阶段拆：执行前（信息）/ 执行中（工具，含 SKILL）/ 执行后（评测）/ 结束（记忆）。
强调：Harness 没有一个统一标准，这两种划分都只是看问题的角度，重要的是能把"反复出现的问题"外化进去。
...
落一个具体数字感：SOL-ExecBench L1-001 上平均加速 **8.93×**、**41 轮迭代**、总耗时 **~2h**——这不是为了秀结果，而是说明一个完全自动化的评测闭环在真实基准上能跑多远。
口头带过同类系统：autoresearch (Karpathy) / autokernel (RightNow-AI)——说明这不是孤例，而是一类正在被验证的思路。
...
#### 103-108 min | 再往深处想一层：几个开放问题【讲】

不给答案，只把几个我自己在想的问题摆出来，让听众带着走：
- **SKILL 的转变**：SKILL 正在变成 harness 的一部分——按需生成、既固化又开放，但天然带着自然语言的不确定性
- **AI 辅助人类 ↔ 人类辅助 AI**：角色正在翻转，不只看结果，也要看过程里谁在盯着谁
- **自动 Harness 优化**：能不能让 agent 自己改自己的 harness？
- **Meta-Harness**：在更高一层，harness 能否反哺模型——即我们搭 harness 探出来的能力边界，最后会不会回写进下一代模型
